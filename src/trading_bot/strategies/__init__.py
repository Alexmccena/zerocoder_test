...
def build_strategy(*, config: AppSettings, runtime_state_provider: Callable[[], RuntimeState]):
    ...
    if config.strategy.name == "grid_dca_v1":
        return GridDcaV1Strategy(config=config, runtime_state_provider=runtime_state_provider)
    raise ValueError(f"unknown strategy configured: {config.strategy.name}")


__all__ = ["Phase3PlaceholderStrategy", "SmcScalperV1Strategy", "GridDcaV1Strategy", "build_strategy"]
